...
class DatagouvObject:
    ...
    def namespace(cls) -> str:
        return f"{cls.CLASS_NAME.lower()}s"

    @staticmethod
    def class_from_name(name: str) -> type["DatagouvObject"]:
        # TODO: check if it's a valid DatagouvObject
...
class TopicObject(DatagouvObject):
    """Objects that can be part of a topic should inherit this class"""

    @classmethod
    def classname(cls) -> str:
        return cls.CLASS_NAME.capitalize()
...
class DatagouvApi:
    ...
    def put_topic_elements(
        ...
        for batch in batches:
            data = [{"element": {"class": object_class.classname(), "id": id}} for id in batch]
            if not self.dry_run:
