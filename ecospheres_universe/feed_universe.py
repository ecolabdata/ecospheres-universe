...
def get_upcoming_universe_perimeter(
    ...
    def _update_perimeter(ids: list[str], orgs: list[Organization]):
        ...
        universe_orgs.update(
            UniverseOrg(id=org.id, name=org.name, slug=org.slug, type=entry.category) for org in orgs
        )

    for entry in grist_entries:
        if entry.object_class in (Dataset, Dataservice):
            if object_class != entry.object_class:
                continue
            # obj = datagouv.get_object(entry.identifier)
            # org from obj
            # _update_perimeter([id], [org])
            pass
        elif entry.object_class is Keyword:
            # objs = datagouv.get_keyword_objects(entry.identifier, object_class)
            # orgs from objs
    ...
    try:
        ...
        for object_class in Topic.object_classes():
            ...
            write_organizations_file(
                conf.output_dir / f"organizations-{object_class.namespace()}-{conf.env}.json",
